...
public class GridLayoutEditor : Editor {
    GridLayout grid;
	string[] selectOptions = {"横排列", "竖排列", "田字格排列", "页面排列", "纵排列"};

	public override void OnInspectorGUI ()
	{
		grid = (GridLayout)target;
		
		grid.arrangement = (GridLayout.Arrangement)EditorGUILayout.Popup("排列类型 : ", (int)grid.arrangement, selectOptions);
	...
	void DrawDiffGUI(int index)
	{
		...
		case 3:
			...
			break;
		case 4:
			grid.cellZ = EditorGUILayout.FloatField("单项深度",grid.cellZ);
			grid.IsInOrder = EditorGUILayout.Toggle("顺序排列", grid.IsInOrder);
			grid.IsMiddleInOrder = EditorGUILayout.Toggle("是否中间排列", grid.IsMiddleInOrder);
			break;
		}
